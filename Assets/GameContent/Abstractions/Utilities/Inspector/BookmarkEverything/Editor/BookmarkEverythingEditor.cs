                    ...
                    if (evt.type == EventType.DragPerform)
                    {
                        ...
                        foreach (Object draggedObject in DragAndDrop.objectReferences)
                        {
                            if (!AssetDatabase.Contains(draggedObject))
                            {
                                EditorUtility.DisplayDialog("Bookmark Everything",
                                    "Objects from hierarchy is not supported for now. Would you like me to add that? Please e-mail me at dogukanerkut@gmail.com.", "Okay");
                                return;
                            }
                        ...
                        if (duplicateList.Count > 0)
                        {
                            ...
                            if (EditorUtility.DisplayDialog("Bookmark Everything",
                                string.Format("Duplicate Entries Found: {0} Would you still like to add them ?(Non-duplicates will be added anyway)", sb.ToString()), "Yes", "No"))
                            {
        ...
        private void DrawProjectFinderEntries(string category)
        {
            ...
            for (int i = 0; i < _currentSettings.EntryData.Count; i++)
            {
                if (_currentSettings.EntryData[i].Category == category)
                {
                    ...
                    //OpenAsset Button
                    if (DrawButton("Open", null, ButtonTypes.Standard))
                    {
        ...
        private void DrawSettings()
        {
            ...
            for (int i = 0; i < _tempLocations.Count; i++)
            {
                ...
                EditorGUILayout.BeginHorizontal();
                {
                    ...
                    _tempLocations[i].Index = EditorGUILayout.Popup(_tempLocations[i].Index, RetrieveGUIContent(_projectFinderHeaders), _popupStyle,
                        GUILayout.MinHeight(EditorGUIUtility.singleLineHeight), GUILayout.MaxWidth(150));
